...
# g.parse( "source/Datasets/lubm-mkg-1.ttl")
# print(len(g))
...
def run_exp(g, sg, index):
    ...
    title = "***** START VALIDATION ON [lubm-mkg-1][schema"+ index +"] *****"
    print(title)
    ...
    v_g1.serialize(destination="Outputs/lubm/mkg1/validationGraph/pyshacl_results[schema"+ index +"].ttl")

    # Saving the validation report in txt

    file = open("Outputs/lubm/mkg1/validationReports/pyshacl_results[schema"+ index +"].txt", "w")
    file.write(v_t1)
    file.close()
    ...
    v_g2.serialize(destination="Outputs/lubm/mkg1/validationGraph/re-shacl_results[schema"+ index +"].ttl")

    # Saving the validation report in txt

    file = open("Outputs/lubm/mkg1/validationReports/re-shacl_results[schema"+ index +"].txt", "w")
    file.write(v_t2)
    file.close()
    ...
    v_g3.serialize(destination="Outputs/lubm/mkg1/validationGraph/pyshacl-rdfs_results[schema"+ index +"].ttl")

    # Saving the validation report in txt

    file = open("Outputs/lubm/mkg1/validationReports/pyshacl-rdfs_results[schema"+ index +"].txt", "w")
    file.write(v_t3)
    file.close()
    ...
    file_table = open("Outputs/lubm/mkg1/RunTimeResults.txt", "a+")
