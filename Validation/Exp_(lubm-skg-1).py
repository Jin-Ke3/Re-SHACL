...
# g.parse( "source/Datasets/lubm-skg-1.ttl")
# print(len(g))
#
...
with open("source/Datasets/lubm-skg-1.pkl", 'rb') as file:
    g = pickle.load(file)
...
def run_exp(g, sg, index):
    ...
    title = "***** START VALIDATION ON [lubm-skg-1][schema"+ index +"] *****"
    print(title)
    ...
    v_g1.serialize(destination="Outputs/lubm/skg1/validationGraph/pyshacl_results[schema"+ index +"].ttl")

    # Saving the validation report in txt

    file = open("Outputs/lubm/skg1/validationReports/pyshacl_results[schema"+ index +"].txt", "w")
    file.write(v_t1)
    file.close()
    ...
    v_g2.serialize(destination="Outputs/lubm/skg1/validationGraph/re-shacl_results[schema"+ index +"].ttl")

    # Saving the validation report in txt

    file = open("Outputs/lubm/skg1/validationReports/re-shacl_results[schema"+ index +"].txt", "w")
    file.write(v_t2)
    file.close()
    ...
    v_g3.serialize(destination="Outputs/lubm/skg1/validationGraph/pyshacl-rdfs_results[schema"+ index +"].ttl")

    # Saving the validation report in txt

    file = open("Outputs/lubm/skg1/validationReports/pyshacl-rdfs_results[schema"+ index +"].txt", "w")
    file.write(v_t3)
    file.close()
    ...
    file_table = open("Outputs/lubm/skg1/RunTimeResults.txt", "a+")
