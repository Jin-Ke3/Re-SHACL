...
g.parse( "source/Datasets/EnDe-30(without_Ontology).ttl")
...
# Preheating with 5 rounds
i = 0
for i in range(5):
    conform, v_g, v_t = validate(g, shacl_graph=sg, inference='none')  


print("***** START VALIDATION ON [EnDe-30] *****")
...
v_g1.serialize(destination="Outputs/EnDe-30/violationGraph/pyshacl_results.ttl")

# Saving the validation report in txt

file = open("Outputs/EnDe-30/validationReports/pyshacl_results.txt", "w")
file.write(v_t1)
file.close()
...
v_g2.serialize(destination="Outputs/EnDe-30/violationGraph/re-shacl_results.ttl")

# Saving the validation report in txt

file = open("Outputs/EnDe-30/validationReports/re-shacl_results.txt", "w")
file.write(v_t2)
file.close()
...
# v_g3.serialize(destination="Outputs/EnDe-30/violationGraph/pyshacl-owl_results.ttl")

# # Saving the validation report in txt

# file = open("Outputs/EnDe-30/validationReports/pyshacl-owl_results.txt", "w")
# file.write(v_t3)
# file.close()
...
file_table = open("Outputs/EnDe-30/RunTimeResults.txt", "a+")
